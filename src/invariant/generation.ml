...
let gen_inv_for_names ctx env names =
  ...
  let vars = List.filter_map (get_var_opt env) names in
  let bools = List.filter is_bool_s vars in
  let ints = List.filter is_int_s vars in
  let reals = List.filter is_real_s vars in
